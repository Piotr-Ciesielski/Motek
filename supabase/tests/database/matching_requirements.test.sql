...
select throws_ok($$ update public.patterns set matching_requirements = '{"version":2,"variants":[{"id":"test","label":"Test","requirements":[{"role":"główna","measurement_basis":"meters","meters_min":100,"materials":["wełna"],"material_match":"all","color_mode":"same","weight_classes":["fingering"],"held_together_group":"oslo-hat"}]}]}'::jsonb where id = (select id from public.patterns limit 1) $$, 'P0001', 'held_together_group nie jest obsługiwane', 'nieobsługiwana grupa nitek jest odrzucana');
