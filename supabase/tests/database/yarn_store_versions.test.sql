...
select is(has_sequence_privilege('authenticated', 'public.yarns_id_seq', 'UPDATE'), false, 'authenticated nie może zmienić sekwencji identyfikatorów');
...
select is(has_sequence_privilege('anon', 'public.yarns_id_seq', 'UPDATE'), false, 'anon nie może zmienić sekwencji identyfikatorów');
...
set local role anon;
select throws_ok('select * from public.yarns', '42501', NULL, 'anon nie może wykonać SELECT na magazynie');
select throws_ok($$ insert into public.yarns (user_id, name, color, material, materials, weight_class, length_meters, weight_grams) values ('11111111-1111-1111-1111-111111111111', 'Anon', 'biały', 'wełna', array['wełna'], 'dk', 100, 50) $$, '42501', NULL, 'anon nie może wykonać INSERT na magazynie');
select throws_ok($$ update public.yarns set name = 'Anon' where user_id = '22222222-2222-2222-2222-222222222222' $$, '42501', NULL, 'anon nie może wykonać UPDATE na magazynie');
select throws_ok($$ delete from public.yarns where user_id = '22222222-2222-2222-2222-222222222222' $$, '42501', NULL, 'anon nie może wykonać DELETE na magazynie');
select throws_ok($$ select nextval('public.yarns_id_seq') $$, '42501', NULL, 'anon nie może użyć sekwencji identyfikatorów');
select throws_ok($$ select last_value from public.yarns_id_seq $$, '42501', NULL, 'anon nie może odczytać sekwencji identyfikatorów');
select throws_ok($$ select setval('public.yarns_id_seq', 1) $$, '42501', NULL, 'anon nie może zmienić sekwencji identyfikatorów');
select throws_ok('select public.get_yarn_store_version()', '42501', NULL, 'anon nie może wykonać RPC get');
select throws_ok($$ select public.insert_yarn_versioned(0, 'Anon', 'biały', array['wełna'], 'dk', 100, 50) $$, '42501', NULL, 'anon nie może wykonać RPC insert');
select throws_ok($$ select public.update_yarn_versioned(0, 0, 'Anon', 'biały', array['wełna'], 'dk', 100, 50) $$, '42501', NULL, 'anon nie może wykonać RPC update');
select throws_ok('select public.delete_yarn_versioned(0, 0)', '42501', NULL, 'anon nie może wykonać RPC delete');
reset role;

set local role authenticated;
select set_config('request.jwt.claim.sub', '11111111-1111-1111-1111-111111111111', true);
select throws_ok($$ insert into public.yarns (user_id, name, color, material, materials, weight_class, length_meters, weight_grams) values ('11111111-1111-1111-1111-111111111111', 'Bez RPC', 'biały', 'wełna', array['wełna'], 'dk', 100, 50) $$, '42501', NULL, 'authenticated nie może wykonać bezpośredniego INSERT');
select throws_ok($$ update public.yarns set name = 'Bez RPC' where user_id = '22222222-2222-2222-2222-222222222222' $$, '42501', NULL, 'authenticated nie może wykonać bezpośredniego UPDATE');
select throws_ok($$ delete from public.yarns where user_id = '22222222-2222-2222-2222-222222222222' $$, '42501', NULL, 'authenticated nie może wykonać bezpośredniego DELETE');
select throws_ok($$ select nextval('public.yarns_id_seq') $$, '42501', NULL, 'authenticated nie może użyć sekwencji identyfikatorów');
select throws_ok($$ select last_value from public.yarns_id_seq $$, '42501', NULL, 'authenticated nie może odczytać sekwencji identyfikatorów');
select throws_ok($$ select setval('public.yarns_id_seq', 1) $$, '42501', NULL, 'authenticated nie może zmienić sekwencji identyfikatorów');
...
select throws_ok($$ select public.update_yarn_versioned(1, (select id from public.yarns where name = 'Zmieniona włóczka'), 'Konflikt', 'zielony', array['wełna'], 'dk', 130, 55) $$, 'P0003', NULL, 'stara wersja jest odrzucana konfliktem');
...
select throws_ok($$ select public.update_yarn_versioned(2, current_setting('test.other_yarn_id')::bigint, 'Cudza zmiana', 'granat', array['wełna'], 'dk', 100, 50) $$, 'P0002', NULL, 'RPC nie pozwala zmienić cudzego rekordu');
...
set local role authenticated;
select set_config('request.jwt.claim.sub', '11111111-1111-1111-1111-111111111111', true);
select throws_ok($$ select public.insert_yarn_versioned(3, 'Ponad limit', 'szary', array['wełna'], 'dk', 100, 50) $$, 'P0001', NULL, 'RPC insert odrzuca 501. włóczkę');
