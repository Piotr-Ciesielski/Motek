...
select throws_ok(
  $$ insert into public.patterns (source_filename, name, description, publication_status, matching_requirements)
     values ('test-published-missing-audit.pdf', 'Test', null, 'published', '{"version":2,"variants":[{"id":"test","label":"Test","requirements":[{"role":"główna","measurement_basis":"meters","meters_min":100,"materials":["wełna"],"material_match":"all","color_mode":"same","weight_classes":["fingering"]}]}]}'::jsonb) $$,
  '23514',
...
select lives_ok(
  $$ insert into public.patterns (source_filename, name, description, publication_status, matching_requirements, content_audit_version, content_audited_at)
     values ('test-published-with-audit.pdf', 'Test', null, 'published', '{"version":2,"variants":[{"id":"test","label":"Test","requirements":[{"role":"główna","measurement_basis":"meters","meters_min":100,"materials":["wełna"],"material_match":"all","color_mode":"same","weight_classes":["fingering"]}]}]}'::jsonb, 'v1', now()) $$,
