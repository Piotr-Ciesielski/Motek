...
declare
  ...
  quantity_field text;
begin
  if jsonb_typeof(new.matching_requirements) is distinct from 'object'
     or not (new.matching_requirements ? 'version')
     or jsonb_typeof(new.matching_requirements->'version') is distinct from 'number'
     or new.matching_requirements->>'version' is distinct from '2'
     or not (new.matching_requirements ? 'variants')
     or jsonb_typeof(new.matching_requirements->'variants') is distinct from 'array' then
  ...
  loop
    if jsonb_typeof(variant) is distinct from 'object' then
      raise exception using errcode = 'P0001', message = 'Wariant musi być obiektem.';
    end if;

    if not (variant ? 'id')
       or jsonb_typeof(variant->'id') is distinct from 'string'
       or btrim(variant->>'id') = ''
    ...
    if not (variant ? 'label')
       or jsonb_typeof(variant->'label') is distinct from 'string'
    ...
    if not (variant ? 'requirements')
       or jsonb_typeof(variant->'requirements') is distinct from 'array'
    ...
    loop
      if jsonb_typeof(requirement) is distinct from 'object' then
        raise exception using errcode = 'P0001', message = 'Rola musi być obiektem.';
      end if;
      if not (requirement ? 'role')
         or jsonb_typeof(requirement->'role') is distinct from 'string'
      ...
      if not (requirement ? 'measurement_basis')
         or (measurement_basis is distinct from 'meters'
             and measurement_basis is distinct from 'grams') then
      ...
      if not (requirement ? 'material_match')
         or (material_match is distinct from 'all'
             and material_match is distinct from 'any'
             and material_match is distinct from 'any_material') then
        raise exception using errcode = 'P0001', message = 'Rola ma nieprawidłowy tryb materiału.';
      end if;
      if not (requirement ? 'materials')
         or jsonb_typeof(requirement->'materials') is distinct from 'array' then
      ...
      if not (requirement ? 'color_mode')
         or (requirement->>'color_mode' is distinct from 'same'
             and requirement->>'color_mode' is distinct from 'any') then
        raise exception using errcode = 'P0001', message = 'Rola ma nieprawidłowy tryb koloru.';
      end if;

      if not (requirement ? 'weight_classes')
         or jsonb_typeof(requirement->'weight_classes') is distinct from 'array'
