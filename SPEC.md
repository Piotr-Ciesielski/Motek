...
**Model danych.** Dodać `public.projects` z `id`, `user_id`, nullable `pattern_id`, `variant_id`, `status`, `version`, `created_at`, `updated_at` i `ended_at`. `user_id` jest kluczem obcym do `auth.users` z `ON DELETE CASCADE`, a `pattern_id` jest kluczem obcym do `public.patterns` z `ON DELETE SET NULL`; wtedy panel aktywnego projektu pokazuje „wzór niedostępny”. `status` ma `CHECK (status IN ('active', 'completed', 'frogged'))`, `version` ma `CHECK (version >= 1)`, a ograniczenie spójności wymaga `ended_at IS NULL` tylko dla `active` i `ended_at IS NOT NULL` tylko dla statusów terminalnych. Dodać `public.project_yarns` o kluczu głównym `(project_id, yarn_id)` oraz polach `role`, `initial_length_meters` i `initial_weight_grams`; obie wartości początkowe są dodatnie. `project_id` jest kluczem obcym z `ON DELETE CASCADE`, a `yarn_id` kluczem obcym z `ON DELETE RESTRICT`. Indeks częściowo unikalny na `projects(user_id)` dla `status = 'active'` jest jedyną regułą jednego aktywnego projektu.

**Autoryzacja i zapis.** RLS pokazuje wyłącznie własne projekty i przypisania, a anonim nie ma dostępu. Bezpośrednie mutacje tabel są odebrane; `SECURITY DEFINER` RPC ma pusty `search_path`, jawną kontrolę `auth.uid()` i minimalny grant `authenticated`. Po legal gate backend tworzy user-scoped klienta Supabase z access tokenem sesji, jak dla magazynu. Node ponownie wylicza pełne dopasowanie, przekazuje do RPC wyłącznie serwerowo wyznaczone `yarn_id` i role; RPC waliduje własność oraz wersję magazynu i nie udaje, że wykonuje matcher SQL.
...
**Kolejność TDD.** Najpierw testy polityki i routera wymagają legal gate, poprawnych identyfikatorów, `If-Match: "yarn-vM"`, `200` i ETag dla aktywnego projektu, `204` bez ETag przy jego braku, 428 dla braku lub złego formatu nagłówka oraz 409 dla nieaktualnej wersji magazynu. Następnie testy bazy wymagają częściowego indeksu, RLS/ACL, własności, `RESTRICT`, `ON DELETE SET NULL`, kaskady konta, serwerowego wyznaczenia przypisań i odrzucenia drugiego aktywnego projektu. Potem zmienić dokument prawa i jego testy, a po minimalnej migracji i RPC dodać router oraz panel.
