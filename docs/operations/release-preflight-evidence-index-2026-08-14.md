...
## Statystyki wywołań legacy RPC — świeży odczyt read-only, 2026-08-15

Odczyt `extensions.pg_stat_statements` nie zmieniał statystyk ani konfiguracji:

- Production ma 8 dopasowanych wpisów i 8 wywołań, ale wszystkie rozpoznane
  zapytania dotyczą definicji, komentarzy oraz `GRANT`/`REVOKE`; nie znaleziono
  wzorca wywołania przez PostgREST;
- Staging ma 11 dopasowanych wpisów i 14 wywołań; oprócz zapytań DDL znajduje
  się wpis PostgREST z 4 wywołaniami legacy RPC, wykonanymi pod rolą
  `authenticated`;
- statystyki stagingowe są utrzymywane co najmniej od `2026-08-03 16:18:39 UTC`,
  ale nie podają ostatniego wykonania ani adresu/klienta, więc nie pozwalają
  rozstrzygnąć, czy wywołania pochodziły z aplikacji, testu, migracji czy
  ręcznego procesu;
- brak wpisu w statystykach nie byłby dowodem braku klienta poza okresem życia
  statystyk.

Wniosek: Production nie ma potwierdzonego bieżącego klienta legacy RPC, ale
stagingowy dowód historycznego użycia przez API oznacza, że bramka „brak
zewnętrznych klientów” pozostaje `OPEN` i ma podwyższone znaczenie. Nie
