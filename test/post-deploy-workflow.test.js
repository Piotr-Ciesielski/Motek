...
test('post-deploy workflow accepts only successful deployments from this repository for approved environment/ref pairs', () => {
  ...
  assert.match(workflow, /github\.event\.deployment\.environment == 'staging'\s*&&\s*github\.event\.deployment\.ref == 'staging'/);
  assert.match(workflow, /github\.event\.deployment\.environment == 'production'\s*&&\s*github\.event\.deployment\.ref == 'main'/);
});
